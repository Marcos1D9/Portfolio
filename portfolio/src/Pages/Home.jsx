            ...
            <div className="w-full flex flex-col justify-center sticky top-0 z-10">
                ...
                <header className={`shrink-0 flex flex-row items-center justify-between py-1 px-auto z-20 bg-white dark:bg-gray-800 bg-opacity-80 dark:bg-opacity-80 backdrop-blur-lg`}>
                    ...
                    <div style={scrollPast ? bounceInStyle : null} className={`tablet:ml-10 flex items-center font-semibold text-lg text-center text-slate-600 dark:text-slate-300 transform transition-all duration-400 ${scrollPast ? '' : 'opacity-0'}`}>
                        ...
                        <h2 className="text-center max-phone:text-[3vw] font-bold">Damián Medina <span className="font-normal">alias</span> "DamiDev"</h2>
                        ...
                        {isSidebarOpen && (
                            ...
                            <div className={`absolute top-0 right-0 w-fit h-fit bg-white dark:bg-gray-900 bg-opacity-95 dark:bg-opacity-95 backdrop-blur-sm shadow-lg z-10 px-5 rounded-bl-3xl transform transition-transform duration-1000 ease-in-out origin-right`}>
                                ...
                                <section className="flex flex-col items-center gap-4">
                                    {/* "Settings" title */}
                                    <p className="mt-4 max-phone:text-[3vw] dark:text-gray-100 text-slate-700 font-bold">{language === "en" ? "Settings" : 'Ajustes'}</p>

                                    <hr className="border-1 w-full h-0 border-gray-600" />
                                    {/* Botón de cambiar tema */}
                                    <button
                                        className="w-fit flex items-center gap-4 outline-none dark:border-gray-700 dark:border-2 border-2 border-gray-200 shadow-md rounded-md"
                                        onClick={handleChangeTheme}
                                    >
                                        <div className="dark:hidden flex justify-between w-fit items-center">
                                            {/* Color theme title text */}
                                            <p className="w-full max-phone:text-[3vw] relative px-3 py-2 active:bg-slate-100 hover:scale-105 dark:hover:scale-105 dark:active:scale-100 text-black rounded-md shadow-md dark:text-white dark:hover:bg-slate-700 transition duration-300">{language === "en" ? "Switch to dark theme" : 'Cambiar a tema oscuro'}</p>

                                            {/* Color theme button */}
                                            <div className="text-3xl max-phone:text-[3vw] dark:hidden text-slate-700">
                                                <FontAwesomeIcon icon={faMoon} />
                                            </div>
                                        </div>
                                        <div className="hidden dark:flex dark:w-fit dark:items-center dark:gap-2">
                                            {/* Color theme title text */}
                                            <p className="w-full max-phone:text-[3vw] relative px-3 py-2 active:bg-slate-100 hover:scale-105 dark:hover:scale-105 dark:active:scale-100 text-black  rounded-md shadow-md dark:text-white dark:hover:bg-slate-700 transition duration-300">{language === "en" ? "Switch to light theme" : 'Cambiar a tema claro'}</p>
            ...
            <div className={` max-w-4xl flex-col mb-5 flex gap-10`}>
                <main className="flex flex-col gap-12">
                    ...
                    <section className="px-[8vw] flex flex-col tablet:flex-column items-center gap-5 shrink-0" >
                        ...
                        {/* Profile Pic */}
                        <div className="shrink-0 relative max-phone:w-[30vw] max-phone:h-[30vw] w-[35%] h-[35%] bg-none shadow-lg border-4 shadow-indigo-400 border-black rounded-full overflow-hidden dark:shadow-indigo-950 dark:drop-shadow-2xl dark:bg-none dark:border-black ">
                            <span>
                        ...
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <div className="flex flex-col items-center gap-2 max-w-full mt-1">
                                <h1 className="shrink-0 text-3xl max-phone:text-[7vw] text-center font-bold text-slate-600 dark:text-slate-100">Marcos Damián E. Medina</h1>
                                <p className="shrink-0 mt-3 text-center hyphens-auto text-slate-700 text-2xl max-phone:text-[5vw] dark:text-slate-200 max-w-full font-Jersey10PixelFont">{language === 'en' ? 'Welcome!' : '¡Bienvenidos!'}</p>
                                <p className="shrink-0 text-center text-slate-700 text-sm max-phone:text-[3vw] dark:text-slate-300 max-w-full h-10 rounded-2xl">{language === 'en' ? 'I am passionate about web development, focused on the integration of social networks' : 'Soy un apasionado del desarrollo de páginas web, enfocado en la integración de redes sociales'}</p>
                            </div>
                            {/* Presentation title */}

                            {/* Social media Buttons start */}
                            <div className="flex gap-3 mt-3 -mb-10 flex-wrap max-phone:mt-4 max-phone:-mb-5">
                                <TooltipButton
                            ...
                            <div className="w-full mt-10 max-phone:mt-5">
                                <LanguageToggleButton toggleLanguage={toggleLanguage} language={language} />
                            </div>
                            {/* Language button end */}
                        </div>
                    ...
                    <section className="px-[8vw] w-full flex flex-col items-center shrink-0 mt-3 mb-3" id="hero-section">
                        ...
                        <header className="flex shrink-0 font-bold text-4xl max-phone:text-[7vw] underline underline-offset-4 decoration-solid text-indigo-400 dark:text-gray-100 mb-12 max-phone:mb-1 max-phone:mx-0 gap-4 items-end">
                        ...
                        <div className="shrink-0  flex flex-col gap-10">
                            <div className="flex flex-col gap-5 pt-8">
                                ...
                                <div className="flex flex-col gap-1">
                                    <h2 className="text-lg max-phone:text-[4vw] font-semibold text-slate-700 dark:text-slate-200">{language === 'en' ? 'Front-End Developer • Munia - Mental and Emotional Health at Work' : 'Desarrollador web Frontend • Munia - Salud Mental y Emocional en el Trabajo'}</h2>
                                    <p className="text-sm max-phone:text-[3vw] text-slate-500 dark:text-slate-400">Sept. 2023 - Nov. 2023</p>
                                </div>
                                {/* Experience Job title end */}


                                {/* Experience Job description */}
                                <ul className="list-disc flex flex-col gap-2 ml-8 mb-12 text-slate-700 dark:text-gray-300 text-md max-phone:text-[3vw]">
                                ...
                                {/* Experience Job title start */}
                                <div className="flex flex-col gap-1 ">
                                    <h2 className="text-lg max-phone:text-[4vw] font-semibold text-slate-700 dark:text-slate-200">{language === 'en' ? 'Front-End Developer • Ingenium - Financial management and administration' : 'Desarrollador web Frontend • Ingenium - Gestión y administración financiera'}</h2>
                                    <p className="text-sm max-phone:text-[3vw] text-slate-500 dark:text-slate-400">Sept. 2024 - Nov. 2024</p>
                                </div>
                                {/* Experience Job title end */}


                                {/* Experience Job description */}
                                <ul className="list-disc flex flex-col gap-2 ml-8 text-slate-700 dark:text-gray-300 text-md max-phone:text-[3vw]">
                    ...
                    <section className="w-full max-phone:w-[fit] flex flex-col items-center mt-2">
                        ...
                        <header className="flex font-bold text-center underline underline-offset-4 text-4xl max-phone:text-[7vw] text-indigo-400 dark:text-gray-100 mb-12 max-phone:mb-8 max-phone:pl-3 max-phone:min-w-full max-phone:justify-center max-phone:px-0 items-center gap-3">
                        ...
                        {/* My projects */}
                        <div className="w-full max-tablet:w-[80%] max-phone:w-[80%] flex flex-col items-center gap-10 max-phone:text-[3vw]">
                            ...
                            <div className="grid gap-10">
                                ...
                                <article className="relative flex flex-col justify-between w-full ">
                                    ...
                                    <div className="flex flex-row mb-3 text-2xl">
                                        <TooltipButton
                                            ...
                                            buttonTextColor="dark:text-white text-white text-stroke text-stroke-sky-400 max-phone:text-stroke-none max-phone:dark:text-stroke-none max-phone:dark:text-sky-300 dark:text-stroke-[1px] dark:text-stroke-sky-400"
                                ...
                                <article className="relative flex flex-col justify-center w-full ">
                                    ...
                                    <div className="flex flex-col items-center justify-center">
                                        <h2 className="text-gray-700 mr-2 dark:text-gray-200 text-2xl max-phone:text-[5vw] text-center font-bold">{language === 'en' ? 'Current working project "Ingenium.net.ar""' : 'Actualmente trabajando en el proyecto "Ingenium.net.ar"'}</h2>
                                    ...
                                    <div className="flex flex-col mt-1.5 justify-center items-center pt-3 pb-6 mb-10 max-phone:mb-[1vw]">
                                        ...
                                        <div className="flex flex-row items-center gap-1">
                                            ...
                                            <div className="relative group/tooltip">
                                                <img src={tailwindIcon} className="w-8" alt="" />
                                                <span className="absolute pointer-events-none transition-all opacity-0 z-20 bottom-full -translate-y-0 py-1 px-1.5 text-xs left-1/2 -translate-x-1/2 rounded-md whitespace-nowrap text-gray-200 bg-gray-800 dark:bg-gray-700 dark:text-white dark:font-medium before:content-[''] before:absolute before:bg-gray-800 before:rounded-sm before:w-2.5 before:rotate-45 before:h-2.5 before:-bottom-1 before:-z-10 before:left-1/2 before:-translate-x-1/2 before:dark:bg-gray-700 before:dark:gray-800 group-hover/tooltip:opacity-100 group-hover/tooltip:-translate-y-3">CSS</span>
                                            </div>
                                            <div className="relative group/tooltip">
                                                <img src={reactIcon} className="w-8" alt="" />
                    ...
                    <section className="px-[8vw] w-full max-phone:flex max-phone:flex-col max-phone:items-center">
                        {/* Email Title */}
                        <header className="font-bold text-3xl max-phone:text-[5vw] text-indigo-400 dark:text-gray-100 mb-[3vw] flex max-phone:justify-center max-[220px]:justify-start max-[220px]:pl-5 items-center gap-3">
                            <h2>{language === 'en' ? 'Email' : 'Correo electrónico'}</h2>
                            <FontAwesomeIcon icon={faEnvelope} className="scale-110" />
                        </header>
                        {/* Email Title */}
                        <div className="items-center gap-6 justify-between max-phone:w-full flex">
                            {/* Email Input */}
                            <input
                                className="bg-black/5 dark:bg-white/5 w-3/4 max-phone:w-[full] max-phone:h-[8vw] h-14 py-[2vw] px-[2vw] max-phone:text-[3vw] rounded-lg place-items-center text-slate-600 dark:text-slate-300"
                                type="text"
                                defaultValue="marcosdamian054@gmail.com"
                            />
                            {/* Email Input */}
                            {/* Email Buttons */}
                            <div className="max-phone:h-[8vw] h-14">
                                {/* Send email button */}
                                <div className="relative group/tooltip max-phone:text-[3vw] h-full flex flex-col items-center">
                                    <a
                                        href="mailto:marcosdamian054@gmail.com"
                                        target="_blank"
                                        rel="noreferrer"
                                        className="bg-indigo-100 hover:bg-opacity-80 dark:bg-indigo-500 hover:dark:bg-opacity-90 text-indigo-500 dark:text-indigo-100 w-fit h-full max-phone:py-[1px] py-4 px-6 max-phone:px-[4vw] rounded-lg flex items-center"
                                    >
                                        <div className="text-lg max-phone:text-[4vw]">
